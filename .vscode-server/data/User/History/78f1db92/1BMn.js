...
async function preprocessData(audioData) {
  ...
  const opusDecoder =new opus;

  const oggData = Buffer.from(audioData, 'base64');
  const opusDecoder = new opus.Decoder({ rate: 16000, channels: 1, frameSize: 960 });
  const rawPCMData = opusDecoder.decode(oggData);
  const wavData = wav.encode(rawPCMData, { sampleRate: 16000, float: false, bitDepth: 16 });
