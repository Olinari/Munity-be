...
async function loadModel() {
  const model = await speechCommands.create(
    "BROWSER_FFT",
    undefined,
    MODEL_PATH
  );
...
async function transcribeSpeech(audioFile) {
    ...
    new Promise((resolve) => {
      const intervalId = setInterval(async () => {
        const response = await axios.get(
          `https://api.convertio.co/convert/${fileId}/status`
        );
  ...
  try {
    const {
      data: { data },
    } = await axios.post("https://api.convertio.co/convert", {
      input: "base64",
      filename: "killme",
      file: audioFile.data,
      apikey: "1a0957588f129108467d2d74ce586372",
      outputformat: "mp3",
    });
